...
export function useConversations() {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    setError(null);
    try {
      const userConversations = await ConversationService.listUserConversations(user.id);
      ...
      const enrichedConversations = await Promise.all(
        userConversations.data.map(async (conversation) => {
          try {
            // Get participants
            const participantsResult = await ConversationService.getConversationParticipants(conversation.id);
            ...
            const currentUserParticipant = participantsResult.data.find(p => p.userId === user.id);
            const unread = currentUserParticipant?.lastReadAt < conversation?.updatedAt;
  ...
  const getConversation = async (conversationId: string) => {
    ...
    try {
      ...
      // Get participants
      const participantsResult = await ConversationService.getConversationParticipants(conversationId);
      ...
      const messagesResult = await ConversationService.getConversationMessages(conversationId);
      
      // Enrich messages with sender details
      const messagesWithSenders = await Promise.all(
        messagesResult.data.map(async (message) => {
          const senderResult = await client.models.User.get({ id: message.senderId });
  ...
  const addAttachment = async (messageId: string, file: {
    ...
    try {
      ...
      const error = err instanceof Error ? err : new Error('Failed to add attachment');
      setError(error);
  ...
  const markAsRead = async (conversationId: string) => {
    ...
    try {
      const result = await ConversationService.markConversationAsRead(conversationId, user.id);
  ...
  const addParticipant = async (conversationId: string, userId: string) => {
    ...
    setError(null);
    try {
      const result = await ConversationService.addParticipant(conversationId, userId);
  ...
  const removeParticipant = async (conversationId: string, userId: string) => {
    ...
    setError(null);
    try {
      const result = await ConversationService.removeParticipant(conversationId, userId);
